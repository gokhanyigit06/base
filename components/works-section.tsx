        ...
        const fetchData = async () => {
            try {
                ...
                if (projectsRes.ok) {
                    ...
                    setFeaturedProjects(projects.filter((p: any) => p.is_featured));
                }

                // Fetch Section Title from settings
                const settingsRes = await fetch('/api/settings');
                if (settingsRes.ok) {
                    const settings = await settingsRes.json();
                    if (Array.isArray(settings)) {
                        const titleChar = settings.find((s: any) => s.key === 'works_section_title');
                        if (titleChar) setSectionTitle(titleChar.value);
                    }
                }
            } catch (e) {
                console.error("Works section fetch failed", e);
            }
        };
        fetchData();
    }, []);
